const {Token, Prefix1, Prefix2, Prefix3} = require('./config/discord.json')
...
//dev2 = 0;
//music = 0;
fs.readdirSync('./commands/db-script/dev').forEach(() => dev1++);
...
//fs.readdirSync('./commands/dbd.js/music').forEach(() => music++);
//fs.readdirSync('./commands/dbd.js/dev').forEach(() => dev2++);
...
bot.ReadyCommand({
  name: "792720490141057035",
  code: `
I have rebooted!
Version: $getVar[version]
Commands: $getVar[commands]
Developer: $tag[672215920558604332]

$setVar[commands;$sum[$getVar[tv5];$getVar[tv6]]]
$setVar[tv6;$sum[$getVar[tv3];$getVar[tv4]]]
$setVar[tv5;$sum[$getVar[tv1];$getVar[tv2]]]
$setVar[tv4;$sum[$getVar[tvdev1];$getVar[fun]]]
$setVar[tv3;$sum[$getVar[log];$getVar[mod]]]
$setVar[tv2;$sum[$getVar[roles];$getVar[util]]]
$setVar[tv1;$sum[$getVar[verify];$getVar[tickets]]]
$setVar[suggest;${suggest}]
$setVar[fun;${fun}]
$setVar[mod;${mod}]
$setVar[tickets;${tickets}]
$setVar[log;${Logging}]
$setVar[roles;${roles}]
$setVar[util;${utility}]
$setVar[tvdev1;${dev1}]
$setVar[verify;${verify}]
`});

bot.Status({
  0: {
      description: "I like cheese, which is why..", 
      type: "PLAYING" 
  }, 
  1: {
      description: "I wish bots could eat because..", 
      type: "WATCHING" 
  }
}, 20000)
...
/////////////////// Antiraid and moderation variables
bot.Variables({
...
let ticket = "https://discord.com/api/webhooks/794470428268822539/kRUKa2r43uPngV1ZS6jMicaAsplWatVq1RmepLOQZaISZBJSaxZ1V6DVCPaMoCcTc__A"
//TICKET VARS
bot.Variables({
ticketnumber: "0",
...
bot.ExecutableCommand({
  name: "blacklist",
  code: `
  $sendWebhook[${ticket};{title:$replaceText[$replaceText[$message[1];add;Blacklisted User];remove;Whitelisted User]}{description:** **}{field:User:$username[$findUser[$message[2]]]#$discriminator[$findUser[$message[2]]] ($findUser[$message[2]]):yes}{field:Moderator:$username[$authorID]#$discriminator[$authorID] ($authorID):yes}{footer:Eyaqtron Ticket System}{color:RANDOM}]
  `})
  
  bot.ExecutableCommand({
  name: "no-ticket",
  code: `
  $sendWebhook[${ticket};{title:Ticket Un-Marked}{description:** **}{field:Channel:$channelID[]):yes}{field:Moderator:$username[$authorID]#$discriminator[$authorID] ($authorID):yes}{footer:Eyaqtron Ticket System}{color:RANDOM}]
  `})
  
  bot.ExecutableCommand({
  name: "new-owner",
  code: `
  $sendWebhook[${ticket};{title:New Owner}{description:** **}{field:User:$username[$findUser[$message[1]]]#$discriminator[$findUser[$message[1]]] ($findUser[$message[1]]):yes}{field:Moderator:$username[$authorID]#$discriminator[$authorID] ($authorID):yes}{footer:Eyaqtron Ticket System}{color:RANDOM}]
  `})
  ...
  $sendWebhook[${ticket};{title:New Ticket}{description:** **}{field:Owner:$username[]#$discriminator[] ($authorID):yes}{field:Channel:<#$getUserVar[ticketchannel]> ($getUserVar[ticketchannel]):yes}{footer:Eyaqtron Ticket System}{color:RANDOM}]
  `})
  
  bot.ExecutableCommand({
  name: "f-owner",
  code: `
  $sendWebhook[${ticket};{title:Owner Forced}{description:** **}{field:User:$username[$findUser[$message[1]]]#$discriminator[$findUser[$message[1]]] ($findUser[$message[1]]):yes}{field:Channel:<#$channelID[]> ($channelID[]):yes}{field:Moderator:$username[$authorID]#$discriminator[$authorID] ($authorID):yes}{footer:Eyaqtron Ticket System}{color:RANDOM}]
  `})
  ...
  {field:Note:$replaceText[$replaceText[$checkCondition[$getVar[reason]==];true;\`No Note Was Provided\`];false;$getVar[reason]]:no}
  ...
  {footer:Eyaqtron Ticket System}{color:RANDOM}]
  $setVar[reason;]
  $replyIn[15s]
  `})
  ...
  **__Thanks for using the Eyaqtron Ticket System!__**}{footer:Welcome to your ticket!}{color:RANDOM} 
  $setChannelVar[webhook;$createWebhook[$channelID[ticket-$getVar[ticketnumber]];Eyaqtron Ticket System;$userAvatar[792692510480138300]];$channelID[ticket-$getVar[ticketnumber]]]]
  `})
  ...
  $sendWebhook[${ticket};{title:Ticket Achived.}
    ...
    field:Archived By:<@$authorID> ($authorID)}
    {footer:Eyaqtron Ticket System}{color:RANDOM}]
  `})
...
name: "$channelID[]",
code: `
$sendWebhook[$getServerVar[logs];{title:Message Deleted}{description:
}]
...
name: "about",
code: `
$title[Eyaqtron v$getVar[version] | About]
$description[


**__Links__**
{hyper:Support Server:https://discord.gg/RPqNRHsuRk}
{hyper:Twitter:https://www.twitter.com/eyaqtron}
{hyper:Invite Me:https://discord.com/api/oauth2/authorize?client_id=792692510480138300&permissions=1043721463&scope=bot}
]
`})
...
bot.JoinedCommand({
  ...
  $author[$authorID]
  $title[$username[]#$discriminator[] joined!]
  $thumbnail[$authorAvatar[webp;true;512]]
  $color[$getServerVar[joinColor]]
  $description[$replaceText[$replaceText[$checkCondition[$getServerVar[joinMsg]==undefined];false;$replaceText[$replaceText[$replaceText[$replaceText[$replaceText[$getServerVar[joinMsg];{user};$username[]];{mention};<@$authorID>];{server};$serverName[$guildID]];{discrim};$discriminator[]];{members};$membersCount[]]];true;]
...
bot.LeaveCommand({
  ...
  code: `
  $author[$authorID]
  $title[$username[]#$discriminator[] left!]
  $thumbnail[$authorAvatar[webp;true;512]]
  $color[$getServerVar[leaveColor]]
  $description[$replaceText[$replaceText[$checkCondition[$getServerVar[leaveChannel]==undefined];false;$replaceText[$replaceText[$replaceText[$replaceText[$replaceText[$getServerVar[leaveMsg];{user};$username[]];{mention};<@$authorID>];{discrim};$discriminator[]];{members};$membersCount[]];{server};$serverName[$guildID]]];true;]]
...
$author[ID: $authorID]
$title[$replaceText[$replaceText[$isBot[$authorID];true;$username[]#$discriminator[]];false;--> Code: **$getUserVar[verifycode]**]]
...
$color[$replaceText[$replaceText[$isBot[$authorID];true;ff0000];false;$getVar[color]]]
...
`})
bot.onJoined()
/////////////////////////////////////// vvv When User Leaves The Server vvv
bot.LeaveCommand({
...
bot.ExecutableCommand({
  ...
  code: `
  $kick[$authorID;Auto kicked for $getServerVar[totalkickstrikes] false verification code attempts]
  $setUserVar[kickstrikes;0]
